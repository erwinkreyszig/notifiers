...
2. Vocab helper  
    - Gets a random, previously unseen word from a set stored in a database table and sends the word's meaning, usage, and example sentences with translations to a slack channel.   
    - Each user gets their own list so they get tagged for every word that gets sent to the channel. 
    - This is powered by a django rest framework app that has an endpoint that gets post requests from a cron job.
    - Slack's api is again used to send formatted messages.
